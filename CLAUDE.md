...
- **The Mechanism:** "Taming the Shock." Optimize a trajectory so that maximum shear force is produced while minimizing shock wave wakes. 最终在皮肤表面的目标点位置产生200Hz（帕西尼小体敏感频率）频率的脉冲高强度剪切 (可以是目标点剪切强度200Hz变化，也可以是目标点剪切强度大致不变，但是剪切方向200Hz变化)，同时抑制其他位置的剪切强度。从而同时提高触觉强度和精度。
...
- **Output:** The PINN must optimize a trajectory tensor of shape `[200, 2]` representing $(x, y)$ coordinates for the SINGLE focal point at each timestep. No multi-foci allowed due to power constraints. 并且目前设备硬件不支持控制换能器振幅，因此聚焦点必须每个时刻都存在，并且永远以最大幅度输出。
