...
def plot_training_history(history: Dict[str, list], output_dir: str):
    ...
    axes[0, 1].plot(history['pulse'])
    axes[0, 1].set_xlabel('Epoch')
    axes[0, 1].set_ylabel('Pulse Loss')
    axes[0, 1].set_title('Temporal Pulse')

    axes[0, 2].plot(history['silence'])
    axes[0, 2].set_xlabel('Epoch')
    axes[0, 2].set_ylabel('Silence Loss')
    axes[0, 2].set_title('Off-Target Silence')

    axes[0, 3].plot(history['contrast'])
    axes[0, 3].set_xlabel('Epoch')
    axes[0, 3].set_ylabel('Contrast')
    axes[0, 3].set_title('Sidelobe Contrast')

    axes[1, 0].plot(history['efficiency'])
    axes[1, 0].set_xlabel('Epoch')
    axes[1, 0].set_ylabel('Efficiency')
    axes[1, 0].set_title('Energy Efficiency')

    axes[1, 1].plot(history['pre_pulse'])
    axes[1, 1].set_xlabel('Epoch')
    axes[1, 1].set_ylabel('Pre-Pulse')
    axes[1, 1].set_title('Pre-Pulse Suppression')

    axes[1, 2].plot(history['trajectory_length'])
    axes[1, 2].set_xlabel('Epoch')
    axes[1, 2].set_ylabel('Trajectory Length')
    axes[1, 2].set_title('Trajectory Length')

    axes[1, 3].plot(history['target_peak'])
    axes[1, 3].set_xlabel('Epoch')
    axes[1, 3].set_ylabel('Target Peak')
    axes[1, 3].set_title('Target Peak Stress')

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'training_history.png'), dpi=150)

    if 'frequency_loss' in history:
        fig, axes = plt.subplots(1, 2, figsize=(12, 4))
        axes[0].plot(history.get('frequency_loss', []))
        axes[0].set_xlabel('Epoch')
        axes[0].set_ylabel('Frequency Loss')
        axes[0].set_title('Frequency Loss (Off/Target)')

        axes[1].plot(history.get('target_band_power', []), label='Target Band')
        axes[1].plot(history.get('off_band_power', []), label='Off Band')
        axes[1].set_xlabel('Epoch')
        axes[1].set_ylabel('Band Power')
        axes[1].set_title('Band Power (200Hz vs Off-band)')
        axes[1].legend(loc='upper right')

        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'training_history_frequency.png'), dpi=150)
        plt.close()
    plt.close()
